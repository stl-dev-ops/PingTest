#!/usr/bin/env python3
"""
Network Ping Monitor with CSV Logging and Email Alerts
Monitors multiple network devices and logs outages to CSV file
Sends email alerts for outages longer than specified threshold
"""
...
def main():
    ...
    print("🌐 Network Ping Monitor with Email Alerts")
    print("=" * 50)
    print(f"Configuration loaded from config.py")
    print(f"Devices to monitor: {len(devices)}")
    
    # Validate email configuration if alerts are enabled
    if EMAIL_ALERTS_ENABLED:
        if not EMAIL_FROM or not EMAIL_PASSWORD or not EMAIL_TO:
            print("⚠️  WARNING: Email alerts are enabled but email configuration is incomplete!")
            print("Please update the email settings in config.py")
            print("Continuing with monitoring only...")
        else:
